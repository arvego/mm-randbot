...
class VkPost:
    ...
    def attachments_handle_fb(self):
        first_doc = True
        text_link = ''
        text_docs = ''
        text_note = ''
        ...
        def log_extraction(attach_type, url='no url'):
            print("  Successfully extracted {} URL for Facebook: {}\n".format(attach_type, url))
        ...
        for attachment in self.attachments:
            ...
            if attachment['type'] in ['posted_photo', 'graffiti', 'app']:
                attach_url = attachment[attachment['type']]['photo_604']
                self.image_links_fb.append(attach_url)
                log_extraction(attachment['type'], attach_url)
            ...
            if attachment['type'] == 'audio':
                ...
                if attach_url.endswith("audio_api_unavailable.mp3"):
                    ...
                    self.audio_links_fb.append(attach_url)
                    log_extraction(attachment['type'], attach_url)

            if attachment['type'] == 'doc':
                attach_url = attachment['doc']['url']
                if attachment['doc']['ext'] == 'gif':
                    self.gif_links_fb.append(attach_url)
                else:
                    if first_doc:
                        text_docs += "\n— Приложения:\n"
                        first_doc = False
        ...
        self.footer_text_fb = text_poll + text_link + text_docs + text_video + text_audio + text_note + text_page + text_album
