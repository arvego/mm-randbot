...
FILENAME="iamdown.txt"              `#переменная для упавшего бота (из config.py)`
...
`#если бот лежит и не убит нами, или если не запущен вовсе, то воскрешаем его`
    if [ -e $FILENAME ] && [ ! -e $FILENAME_KILL ] || [ $(ps -ef | grep "python3 ./mm-randbot.py" | grep -v "grep" | wc -l) -eq "0" ] && [ ! -e $FILENAME_KILL ]; then
        printf "Bot is down. Ressurecting it.\n-----------------------------\n\n"
        NOTIFIED=0
        NOTIFIED_KILL=0
        python3 mm-randbot.py
